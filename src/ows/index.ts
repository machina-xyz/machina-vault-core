...
import type { VaultKey, KeyScope, KeyPermissions } from "../keys/types.js";
import type { PolicyRule, PolicyEvalRequest, PolicyContext } from "../policy/types.js";
...
/** Raw OWS keystore.json structure (Ethereum Keystore v3) */
interface OwsKeystoreV3 {
  version: 3;
  id: string;
  crypto: {
...
/** Raw OWS metadata.json structure */
interface OwsMetadata {
  name: string;
  createdAt: string;
  chains: string[];
}
 ...
 */
export function readOwsVault(vaultPath: string, password: string): OwsWallet {
  // --- Read raw files ---
  ...
  const keystore: OwsKeystoreV3 = JSON.parse(
    fs.readFileSync(keystorePath, "utf-8"),
  );
  if (keystore.version !== 3) {
    throw new Error(
      `Unsupported OWS keystore version: ${keystore.version} (expected 3)`,
    );
  }

  const config: OwsConfig = fs.existsSync(configPath)
    ? JSON.parse(fs.readFileSync(configPath, "utf-8"))
    : { chains: [{ chainId: "eip155:1", family: "evm" }] };

  const metadata: OwsMetadata = fs.existsSync(metadataPath)
  ...
  for (const chain of config.chains) {
    const derivationPath =
      chain.derivationPath ?? DEFAULT_DERIVATION_PATHS[chain.family] ?? DEFAULT_DERIVATION_PATHS["evm"];
 ...
 * each subdirectory.
 */
export function listOwsVaults(basePath?: string): OwsVaultInfo[] {
 ...
 */
export function importOwsToMachina(
  owsWallet: OwsWallet,
  machinaAgentId: string,
): VaultKey {
 ...
 */
export function wrapOwsSigning(
  owsWallet: OwsWallet,
  ...
  return {
    sign: async (
      ...
      // --- Policy evaluation ---
      const evalRequest: PolicyEvalRequest = {
      ...
      if (!result.allowed) {
        const reasons = result.matchedRules
          ...
          .join("; ");
        throw new Error(
          `MACHINA policy denied OWS signing on chain ${chainId}: ${reasons || result.action}`,
        );
      }

      // --- Sign using OWS key material ---
      const derivationPath =
        getDerivationPathForChain(chainId) ?? DEFAULT_DERIVATION_PATHS["evm"];
    ...
    getAddress: (chainId: string): string => {
      ...
      if (!caip10) {
        throw new Error(
          `OWS wallet "${owsWallet.name}" has no address for chain: ${chainId}`,
        );
      }
 ...
 */
function decryptKeystore(keystore: OwsKeystoreV3, password: string): string {
  ...
  if (computedMac !== ks.mac) {
    throw new Error(
      "OWS keystore decryption failed: invalid password or corrupted keystore",
    );
  }
...
function deriveKeyPair(
  ...
  throw new Error(
    "OWS key derivation requires @scure/bip32 and @noble/curves for real BIP-32 HD derivation. " +
 ...
 */
function signMessage(_privateKey: Uint8Array, _message: Uint8Array): Uint8Array {
  throw new Error(
    "OWS signing requires @noble/curves for real ECDSA (secp256k1) or EdDSA (Ed25519) signatures. " +
 ...
 */
function getDerivationPathForChain(chainId: string): string | undefined {
