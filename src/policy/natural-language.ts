...
import type { PolicyRule, PolicyCondition, PolicyAction } from "./types.js";

let nlIdCounter = 0;

function generateNlId(): string {
  nlIdCounter += 1;
...
function parseWindowSeconds(text: string): number {
...
function parseHour24(hourStr: string, ampm: string): number {
...
function tryBlockOverAmount(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("block") || lower.includes("deny") || lower.includes("reject"))) return null;
  if (!(lower.includes("over") || lower.includes("above") || lower.includes("exceed"))) return null;
...
function tryRequireApprovalOverAmount(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!lower.includes("approv")) return null;
  if (!(lower.includes("over") || lower.includes("above") || lower.includes("exceed"))) return null;
...
function tryRecipientAllowlist(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("only allow") || lower.includes("only send") || lower.includes("only transfer"))) return null;
...
function tryRecipientBlocklist(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("block") || lower.includes("deny") || lower.includes("blacklist"))) return null;
  if (!(lower.includes("to ") || lower.includes("recipient") || lower.includes("address"))) return null;

  const addresses = parseAddresses(input);
  if (addresses.length === 0) return null;

  // Ensure this isn't an "over $X" pattern (handled by tryBlockOverAmount)
  if (lower.includes("over") || lower.includes("above")) return null;
...
function tryVelocityLimit(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("limit") || lower.includes("max") || lower.includes("cap"))) return null;
...
function tryTimeWindow(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("between") || lower.includes("after") || lower.includes("before") || lower.includes("outside"))) return null;
  if (!(lower.includes("am") || lower.includes("pm"))) return null;

  const timeMatches: Array<{ hour: number }> = [];
...
function tryDailyLimit(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!lower.includes("daily") && !lower.includes("per day")) return null;
  if (!(lower.includes("limit") || lower.includes("max") || lower.includes("cap"))) return null;
...
function tryBlockChain(input: string): ParsedIntent | null {
  const lower = input.toLowerCase();
  if (!(lower.includes("block") || lower.includes("deny") || lower.includes("no "))) return null;
  if (!lower.includes("chain")) return null;
...
const PARSERS: Array<(input: string) => ParsedIntent | null> = [
  tryBlockOverAmount,
 ...
 *  - "daily limit of $5000"
 */
export function parseNaturalLanguagePolicy(input: string): PolicyRule | null {
