 ...
 */
function deriveGeneratorH(): InstanceType<typeof secp256k1.ProjectivePoint> {
...
let _generatorH: InstanceType<typeof secp256k1.ProjectivePoint> | null = null;

function getGeneratorH(): InstanceType<typeof secp256k1.ProjectivePoint> {
  if (_generatorH === null) {
 ...
 */
export function createBalanceCommitment(balance: bigint): {
  commitment: string;
  blinding: string;
} {
  if (balance < 0n) {
...
function createCommitmentWithBlinding(
  value: bigint,
  blinding: bigint,
): InstanceType<typeof secp256k1.ProjectivePoint> {
  const H = getGeneratorH();
...
/**
 * Serialised range proof structure:
 * - 1 byte: number of bits
 * - 1 byte: flags (0x01 = has min, 0x02 = has max)
 * - 32 bytes: minBalance (big-endian, 0 if not set)
 * - 32 bytes: maxBalance (big-endian, 0 if not set)
 * - For each bit of (balance - min):
 *   - 33 bytes: bit commitment (compressed point)
 *   - 32 bytes: bit blinding factor
 * - For each bit of (max - balance):
 *   - 33 bytes: bit commitment (compressed point)
 *   - 32 bytes: bit blinding factor
 * - 32 bytes: aggregate blinding factor for lower bound
 * - 32 bytes: aggregate blinding factor for upper bound
 */
...
interface RangeProofData {
  ...
  minBalance: bigint;
  maxBalance: bigint;
  lowerBitCommitments: Uint8Array[];
  lowerBitBlindings: bigint[];
  upperBitCommitments: Uint8Array[];
  upperBitBlindings: bigint[];
  lowerAggregateBlinding: bigint;
  upperAggregateBlinding: bigint;
}
 ...
 */
function createBitDecomposition(
  value: bigint,
  bits: number,
): {
  commitments: Uint8Array[];
  blindings: bigint[];
  aggregateBlinding: bigint;
} {
  if (value < 0n) {
...
function serializeRangeProof(data: RangeProofData): Uint8Array {
...
function deserializeRangeProof(data: Uint8Array): RangeProofData {
  let offset = 0;
  ...
  const lowerBitCommitments: Uint8Array[] = [];
  const lowerBitBlindings: bigint[] = [];
  const upperBitCommitments: Uint8Array[] = [];
  ...
  const lowerAggregateBlinding = bytesToBigInt(
    data.slice(offset, offset + 32),
  );
  offset += 32;
  const upperAggregateBlinding = bytesToBigInt(
    data.slice(offset, offset + 32),
  );
...
export function createBalanceProof(
  ...
  const commitment = createCommitmentWithBlinding(
    request.balance,
    blindingScalar,
  );
...
export function verifyBalanceProof(
  ...
  const result: BalanceProofVerification = {
    valid: false,
  ...
  try {
    ...
    const commitmentPoint = secp256k1.ProjectivePoint.fromHex(
      stripHexPrefix(commitment),
    );
    commitmentPoint.assertValidity();

    // 2. Verify proof commitment matches expected commitment
    const proofCommitmentPoint = secp256k1.ProjectivePoint.fromHex(
      stripHexPrefix(proof.commitment),
    );
    ...
    if (hasMin) {
      rangeValid =
        ...
        verifyBitDecomposition(
          rangeProof.lowerBitCommitments,
          rangeProof.lowerBitBlindings,
          rangeProof.bits,
        );
    }
    ...
    if (hasMax) {
      rangeValid =
        rangeValid &&
        verifyBitDecomposition(
          rangeProof.upperBitCommitments,
          rangeProof.upperBitBlindings,
          rangeProof.bits,
        );
    }
 ...
 */
export function createSolvencyProof(
  balances: Array<{ token: string; balance: bigint }>,
): {
  totalCommitment: string;
  proofs: BalanceProof[];
} {
  const proofs: BalanceProof[] = [];
 ...
 * @returns Verification result
 */
export function verifySolvencyProof(
  totalCommitment: string,
  proofs: BalanceProof[],
): { valid: boolean } {
  try {
    const expectedTotal = secp256k1.ProjectivePoint.fromHex(
      stripHexPrefix(totalCommitment),
    );
