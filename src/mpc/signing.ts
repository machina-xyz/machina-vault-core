 ...
 */
export function signRound1(
  share: MPCKeyShare,
  messageHex: string,
  participantIds: string[],
): { state: SignRound1State; round1Msg: SignRound1Msg } {
  if (participantIds.length < share.threshold) {
    throw new Error(
      `Need at least ${share.threshold} participants, got ${participantIds.length}`,
    );
  }
...
export function signRound2(
  ...
  round1Messages: SignRound1Msg[],
): { state: SignRound2State; round2Msg: SignRound2Msg } {
  const participantIndices = round1Messages.map((msg) => msg.fromIndex);

  const round2State: SignRound2State = {
    ...state,
    participantIndices,
    round1Messages,
  };

  const round2Msg: SignRound2Msg = {
 ...
 */
export function signRound3(
  state: SignRound2State,
  round2Messages: SignRound2Msg[],
): { state: SignRound3State; round3Msg: SignRound3Msg } {
  if (round2Messages.length !== state.round1Messages.length) {
    throw new Error(
      `Mismatch: ${state.round1Messages.length} round 1 msgs vs ${round2Messages.length} round 2 msgs`,
    );
  }

  // Verify each revealed nonce against its round 1 commitment
  for (const r2Msg of round2Messages) {
    const r1Msg = state.round1Messages.find(
      (m) => m.fromParty === r2Msg.fromParty,
    );
    if (!r1Msg) {
    ...
    if (!hashVerify(commitment, nonceBytes, opening)) {
      throw new Error(
        `Nonce commitment verification failed for party ${r2Msg.fromParty}`,
      );
    }
  }
  ...
  const lambda = lagrangeCoefficient(
    state.share.shareIndex,
    state.participantIndices,
  );
  ...
  const round3State: SignRound3State = {
    ...state,
    round2Messages,
    combinedR,
  };

  const round3Msg: SignRound3Msg = {
    fromParty: state.share.partyId,
    fromIndex: state.share.shareIndex,
 ...
 */
export function signRound4(
  state: SignRound3State,
  round3Messages: SignRound3Msg[],
): { result: MPCSignResult; round4Msg: SignRound4Msg } {
  ...
  for (const msg of round3Messages) {
    ...
    if (hex.length !== 128) {
      throw new Error(
        `Invalid partial signature length from party ${msg.fromParty}: expected 128 hex chars, got ${hex.length}`,
      );
    }
...
export function verifySignature(
  ...
  try {
    ...
    if (r >= CURVE_ORDER || s >= CURVE_ORDER) return false;
...
function encodeDER(r: Uint8Array, s: Uint8Array): Uint8Array {
  ...
  const rFinal =
    rTrimmed[0]! >= 0x80 ? new Uint8Array([0, ...rTrimmed]) : rTrimmed;
  const sFinal =
    sTrimmed[0]! >= 0x80 ? new Uint8Array([0, ...sTrimmed]) : sTrimmed;
