...
function base64urlDecode(str: string): Uint8Array {
  ...
  if (remainder === 2) outputLen += 1;
  else if (remainder === 3) outputLen += 2;
  ...
  for (let i = 0; i < len; i += 4) {
    ...
    if (pos < outputLen) out[pos++] = (a << 2) | (b >> 4);
    if (pos < outputLen) out[pos++] = ((b << 4) | (c >> 2)) & 0xff;
    if (pos < outputLen) out[pos++] = ((c << 6) | d) & 0xff;
  }
...
async function hmacVerify(
  ...
  return crypto.subtle.verify("HMAC", key, signature as BufferSource, data as BufferSource);
}
