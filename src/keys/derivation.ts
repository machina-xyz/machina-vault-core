...
export const OPERATOR_PATH = (index: number) =>
  `m/44'/60'/1'/0'/${index}'`;

/** Agent keys live under account 2 */
export const AGENT_PATH = (index: number) =>
  `m/44'/60'/2'/0'/${index}'`;
 ...
 */
function masterNodeFromSeed(seed: Uint8Array): DerivedNode {
...
function parsePath(path: string): number[] {
  ...
  return segments.map((seg) => {
    ...
    if (!hardened) {
      throw new Error(
        `Non-hardened derivation not supported for security: "${seg}"`,
      );
    }
    return index;
  });
 ...
 */
export function deriveMasterSeed(rootEntropy: Uint8Array): Uint8Array {
 ...
 */
export function publicKeyToEvmAddress(compressedPubKey: Uint8Array): string {
 ...
 */
export function deriveOperatorKey(
  masterSeed: Uint8Array,
  index: number,
