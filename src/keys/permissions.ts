...
export function createPermissions(...bits: bigint[]): KeyPermissions {
  let mask = 0n;
...
export function hasPermission(perms: KeyPermissions, bit: bigint): boolean {
...
export function hasAllPermissions(
  ...
  for (const bit of bits) {
    if ((perms.mask & (1n << bit)) === 0n) return false;
  }
  return true;
}
...
export function hasAnyPermission(
  ...
  for (const bit of bits) {
    if ((perms.mask & (1n << bit)) !== 0n) return true;
  }
  return false;
}
 ...
 */
export function constrainPermissions(
  parent: KeyPermissions,
  requested: KeyPermissions,
): KeyPermissions {
...
export const ROOT_PERMISSIONS: KeyPermissions = {
  mask: (1n << 256n) - 1n,
};

/** Operator keys: create/revoke agent + session keys, sign, manage policy, view. */
export const DEFAULT_OPERATOR_PERMISSIONS: KeyPermissions = createPermissions(
  PERM.CREATE_AGENT_KEY,
  PERM.REVOKE_AGENT_KEY,
  PERM.CREATE_SESSION_KEY,
  PERM.REVOKE_SESSION_KEY,
  PERM.SIGN_TRANSACTION,
  PERM.MANAGE_POLICY,
  PERM.VIEW_BALANCES,
  PERM.APPROVE_TRANSACTION,
  PERM.MANAGE_ALLOWLIST,
  PERM.ROTATE_KEYS,
  PERM.VIEW_AUDIT_LOG,
...
export const DEFAULT_AGENT_PERMISSIONS: KeyPermissions = createPermissions(
  PERM.CREATE_SESSION_KEY,
  PERM.REVOKE_SESSION_KEY,
  PERM.SIGN_TRANSACTION,
  PERM.VIEW_BALANCES,
...
export const DEFAULT_SESSION_PERMISSIONS: KeyPermissions = createPermissions(
  PERM.SIGN_TRANSACTION,
);
 ...
 */
const TIER_DEPTH: Record<KeyTier, number> = {
  root: 0,
  operator: 1,
  agent: 2,
  session: 3,
 ...
 *  - session → NOTHING
 */
export function canCreateKeyOfTier(
  parentTier: KeyTier,
  childTier: KeyTier,
): boolean {
 ...
 */
export function requiredPermissionForTier(childTier: KeyTier): bigint {
  switch (childTier) {
    case "operator":
 ...
 */
export function validateKeyCreation(
  parentTier: KeyTier,
  ...
  if (!canCreateKeyOfTier(parentTier, childTier)) {
    throw new Error(
      `Hierarchy violation: "${parentTier}" key cannot create "${childTier}" key`,
    );
  }
  ...
  if (!hasPermission(parentPermissions, required)) {
    throw new Error(
      `Permission denied: parent key lacks CREATE_${childTier.toUpperCase()}_KEY permission`,
    );
  }
}
