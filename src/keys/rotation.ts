 ...
 */
export function shouldRotate(key: VaultKey): boolean {
  if (key.status !== "active") return false;
  if (key.scope.autoRotateInterval === null) return false;
 ...
 */
export function rotateKey(params: RotateKeyParams): RotateKeyResult {
  const { oldKey, masterSeed, newIndex } = params;

  if (oldKey.tier !== "operator" && oldKey.tier !== "agent") {
    throw new Error(
      `Cannot rotate "${oldKey.tier}" key. Only operator and agent keys support rotation.`,
    );
  }

  if (oldKey.status !== "active") {
    throw new Error(
      `Cannot rotate key with status "${oldKey.status}". Key must be "active".`,
    );
  }

  // Derive the new key
  const deriveFn =
    oldKey.tier === "operator" ? deriveOperatorKey : deriveAgentKey;
