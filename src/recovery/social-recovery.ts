...
(function initGF256() {
  ...
  for (let i = 0; i < 255; i++) {
    ...
    if (x >= 256) x ^= 0x11b; // reduce by AES polynomial
  }
...
export function createRecoveryConfig(
  vaultId: string,
  threshold: number,
  guardians: Array<{ identifier: string; type: RecoveryGuardian["type"]; name: string }>,
  options?: { cloudBackup?: boolean; hardwareBackup?: boolean; cooldownSeconds?: number },
): RecoveryConfig {
 ...
 */
export function submitRecoveryShare(
