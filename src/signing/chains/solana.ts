...
function bytesToBase58(bytes: Uint8Array): string {
  ...
  let result = "";
  for (const byte of bytes) {
    if (byte === 0) result += B58_ALPHABET[0];
    else break;
  }
...
function base58ToBytes(str: string): Uint8Array {
  ...
  for (const char of str) {
    if (char === B58_ALPHABET[0]) bytes.push(0);
    else break;
  }
...
function encodeCompactU16(value: number): Uint8Array {
  if (value < 0 || value > 65535) throw new Error(`compact-u16 out of range: ${value}`);
  if (value < 0x80) {
...
export class SolanaSigner implements ChainSigner {
  ...
  async sign(
    ...
    const blockhashResult = (await rpcCallWithRetry(
  ...
  async broadcast(rawTx: string, rpcUrl: string): Promise<BroadcastResult> {
    try {
      ...
      const txHash = (await rpcCallWithRetry(
        rpcUrl,
        "sendTransaction",
        [rawTx, { encoding: "base58", skipPreflight: false }],
      )) as string;
