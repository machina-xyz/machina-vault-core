...
function bcsVec(items: Uint8Array[]): Uint8Array {
...
export class SuiSigner implements ChainSigner {
  ...
  async sign(
    ...
    if (request.data) {
      ...
      const result = (await rpcCallWithRetry(
        ...
        [
          ...
          1_000_000n.toString(), // gas_budget (1M MIST = 0.001 SUI)
          request.to,         // recipient
          amount.toString(),  // amount (in MIST)
        ],
      )) as { txBytes: string; gas: unknown[] };
    ...
    // 3. Sign the hash
    const intentMessage = concat(
      new Uint8Array([INTENT_SCOPE_TRANSACTION, INTENT_VERSION_V0, INTENT_APP_SUI]),
      txBytes,
    );
    ...
    const serializedSig = concat(
      new Uint8Array([ED25519_FLAG]),
      signature,
      publicKey,
    );
  ...
  async broadcast(rawTx: string, rpcUrl: string): Promise<BroadcastResult> {
    try {
      const { txBytes, signature } = JSON.parse(rawTx) as {
        txBytes: string;
        signature: string;
      };
      ...
      return {
        ...
        chain: "sui",
        status: status as "submitted" | "confirmed",
      };
    } catch (err) {
      return {
        txHash: "",
