...
import { base64urlDecode, concatBuffers } from "./utils.js";
...
export function verifyRegistration(
  ...
  try {
    const uncompressedKey = concatBuffers(
      new Uint8Array([0x04]),
...
export function verifyAuthentication(
  ...
  storedCredential: VaultCredential,
  challenge: VaultChallenge,
  expectedOrigin: string,
): boolean {
  ...
  try {
    clientData = JSON.parse(decoder.decode(auth.clientDataJSON)) as {
      type?: string;
      challenge?: string;
      origin?: string;
    };
  ...
  const uncompressedKey = concatBuffers(
    new Uint8Array([0x04]),
    storedCredential.publicKeyX,
    storedCredential.publicKeyY,
  );
...
function uint8ArrayToBase64url(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length; i += 3) {
    ...
    if (i + 1 < bytes.length) result += chars[((b1 & 0x0f) << 2) | (b2 >> 6)]!;
