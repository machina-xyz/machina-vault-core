...
export async function createVaultCredential(
  ...
  const { publicKey, publicKeyX, publicKeyY, hardwareBacked } =
  ...
  const attestationType: "platform" | "cross-platform" =
    transports.includes("internal") ? "platform" : "cross-platform";
...
function parseAuthenticatorData(authData: Uint8Array): {
  ...
  const credIdLen =
    (authData[credIdLenOffset]! << 8) | authData[credIdLenOffset + 1]!;
 ...
 */
function parseCoseP256Key(data: Uint8Array): {
  x: Uint8Array;
  y: Uint8Array;
  coseKey: Uint8Array;
} {
  ...
  if (x.length !== 32 || y.length !== 32) {
    throw new Error(
      `Invalid P-256 key coordinates: x=${x.length} bytes, y=${y.length} bytes (expected 32 each)`,
    );
  }
