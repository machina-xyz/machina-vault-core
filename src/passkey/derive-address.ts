...
export function deriveVaultAddress(
  ...
  if (publicKeyX.length !== 32) {
    throw new Error(
      `Public key X coordinate must be 32 bytes, got ${publicKeyX.length}`,
    );
  }
  if (publicKeyY.length !== 32) {
    throw new Error(
      `Public key Y coordinate must be 32 bytes, got ${publicKeyY.length}`,
    );
  }
...
export function toChecksumAddress(address: string): string {
  ...
  if (stripped.length !== 40) {
    throw new Error(
      `Invalid address length: expected 40 hex chars, got ${stripped.length}`,
    );
  }
