 ...
 */
export async function generateAttestation(platform, nonce) {
    ...
    switch (platform.type) {
        ...
        case "tee":
            return {
                platform: "tee",
                evidence: toHex(new TextEncoder().encode(JSON.stringify({ status: "tee_attestation_pending", platform: "tee" }))),
                timestamp,
                nonce: nonceHex,
                measurements: {
                    status: "pending",
        ...
        case "hsm":
            return {
                platform: "hsm",
                evidence: toHex(new TextEncoder().encode(JSON.stringify({ status: "hsm_attestation_pending", platform: "hsm" }))),
                timestamp,
                nonce: nonceHex,
                measurements: {
                    status: "pending",
 ...
 */
export async function verifyAttestation(report) {
 ...
 */
export function isAttestationFresh(report, maxAgeMs = DEFAULT_MAX_AGE_MS) {
...
// ---------------------------------------------------------------------------
async function generateWebAuthnAttestation(nonce, nonceHex, timestamp) {
    try {
...
async function generateCloudflareAttestation(nonce, nonceHex, timestamp) {
    // Derive a platform-specific HMAC key from fixed seed + nonce
    const seed = new Uint8Array(64);
    crypto.getRandomValues(seed);
    const keyMaterial = await crypto.subtle.importKey("raw", seed, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    // Build the attestation payload: info || timestamp || nonce
    const payload = concatBytes(CF_ATTESTATION_INFO, new TextEncoder().encode(timestamp), nonce);
    const signature = await crypto.subtle.sign("HMAC", keyMaterial, payload);
...
async function generateSoftwareAttestation(platformType, nonce, nonceHex, timestamp) {
    // Self-attestation: HMAC over nonce + timestamp
    const seed = new Uint8Array(32);
    crypto.getRandomValues(seed);
    const keyMaterial = await crypto.subtle.importKey("raw", seed, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const payload = concatBytes(new TextEncoder().encode(`machina-${platformType}-attestation-v1`), new TextEncoder().encode(timestamp), nonce);
    const signature = await crypto.subtle.sign("HMAC", keyMaterial, payload);
    const evidence = concatBytes(seed, new Uint8Array(signature));
    return {
...
// ---------------------------------------------------------------------------
async function verifyWebAuthnAttestation(report) {
    try {
        ...
        return {
            ...
            platform: "webauthn",
            securityLevel: "unknown",
            error: `WebAuthn verification failed: ${err.message}`,
        };
    }
}
async function verifyCloudflareAttestation(report) {
    try {
        const evidenceBytes = fromHex(report.evidence);
        ...
        const keyMaterial = await crypto.subtle.importKey("raw", seed, { name: "HMAC", hash: "SHA-256" }, false, ["verify"]);
        const nonceBytes = fromHex(report.nonce);
        const payload = concatBytes(CF_ATTESTATION_INFO, new TextEncoder().encode(report.timestamp), nonceBytes);
        const valid = await crypto.subtle.verify("HMAC", keyMaterial, signature, payload);
        if (!valid) {
            return {
                valid: false,
                platform: "cloudflare",
        ...
        return {
            ...
            platform: "cloudflare",
            securityLevel: "firmware",
            error: `Cloudflare attestation verification failed: ${err.message}`,
        };
    }
}
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function softwareFallbackReport(platform, nonceHex, timestamp, reason) {
    return {
        platform,
        evidence: toHex(new TextEncoder().encode(JSON.stringify({ status: "fallback", reason }))),
        timestamp,
        nonce: nonceHex,
        measurements: {
            security_level: "software",
