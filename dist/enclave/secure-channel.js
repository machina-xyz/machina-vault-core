 ...
 */
export async function createSecureChannel(ourKeyPair, peerPublicKey) {
    // Import our private key for ECDH
    const privateKey = await crypto.subtle.importKey("pkcs8", ourKeyPair.privateKey, { name: "ECDH", namedCurve: "P-256" }, false, ["deriveBits"]);
    ...
    return {
        async encrypt(plaintext) {
            ...
            const ciphertext = await crypto.subtle.encrypt({ name: "AES-GCM", iv: nonce }, encryptKey, plaintext);
            return {
        ...
        async decrypt(ciphertext, nonce) {
            const plaintext = await crypto.subtle.decrypt({ name: "AES-GCM", iv: nonce }, encryptKey, ciphertext);
 ...
 */
export async function deriveChannelKeys(sharedSecret, info) {
    ...
    const macKey = await crypto.subtle.deriveKey({
        ...
        hash: "SHA-256",
        salt: salt,
        info: new TextEncoder().encode(info === HKDF_ENC_INFO ? HKDF_MAC_INFO : info + "-mac"),
