 ...
 */
export async function createMCPSession(config, agentId, requestedTools, secret) {
    // Filter requested tools to only those allowed by the server config
    const grantedTools = requestedTools.filter((tool) => config.allowedTools.includes(tool));
    ...
    const jwt = await createJWT(tokenPayload, secret, config.maxSessionDurationMs);
    const token = {
        sub: agentId,
        iss: `machina:mcp:${config.serverId}`,
        aud: config.serverId,
 ...
 */
export function validateMCPToolCall(session, toolName) {
 ...
 */
export function createMCPChallenge(serverId) {
 ...
 */
export async function verifyMCPChallenge(challenge, response, serverSecret) {
    // Check expiration
    if (new Date(challenge.expiresAt).getTime() < Date.now()) {
        return false;
    }
    // Compute expected HMAC of the challenge
    const key = await crypto.subtle.importKey("raw", serverSecret, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    const challengeData = hexToBytes(challenge.challenge);
    const expectedSig = new Uint8Array(await crypto.subtle.sign("HMAC", key, challengeData));
