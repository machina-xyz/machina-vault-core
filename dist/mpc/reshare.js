 ...
 */
import { toHex, fromHex, modN, randomBytes, } from "./commitment.js";
import { Point, randomScalar, scalarToHex, hexToScalar, evaluatePolynomial, verifyFeldmanShare, lagrangeCoefficient, } from "./keygen.js";
/**
 * Create a new reshare session. This is the coordination object that
 * all parties reference during the resharing protocol.
 */
export function initiateReshare(config) {
    ...
    if (config.oldParties.length < config.oldThreshold) {
        throw new Error(`Need at least ${config.oldThreshold} old parties for resharing, got ${config.oldParties.length}`);
    }
 ...
 */
export function reshareRound1(oldShare, config) {
    ...
    const commitmentHexes = commitments.map((c) => toHex(c.toRawBytes(true)));
    ...
    for (let j = 0; j < config.newParties.length; j++) {
        ...
        const subShare = evaluatePolynomial(coefficients, BigInt(newPartyIndex));
        round1Messages.push({
 ...
 */
export function reshareRound2(newPartyId, newPartyIndex, round1Messages, config, expectedPublicKey) {
    // Filter messages addressed to this party
    const myMessages = round1Messages.filter((msg) => msg.toParty === newPartyId);
    if (myMessages.length < config.oldThreshold) {
        throw new Error(`Need at least ${config.oldThreshold} sub-shares, got ${myMessages.length}`);
    }
    // Verify each sub-share against Feldman commitments
    for (const msg of myMessages) {
        const subShareValue = hexToScalar(msg.subShare);
        const commitmentPoints = msg.commitments.map((hex) => Point.fromHex(fromHex(hex)));
        // Verify: subShare * G == sum_{j=0}^{t'-1} (newPartyIndex^j * C_j)
        const sharePoint = Point.BASE.multiply(subShareValue);
        const expectedPoint = verifyFeldmanShare(commitmentPoints, BigInt(newPartyIndex));
        if (!sharePoint.equals(expectedPoint)) {
            throw new Error(`Sub-share verification failed for share from party ${msg.fromParty}`);
        }
    }
    ...
    if (!reconstructedPub.equals(expectedPubPoint)) {
        throw new Error("Resharing verification failed: reconstructed public key does not match expected");
    }
 ...
 */
export function reshare(oldShares, config) {
    if (oldShares.length < config.oldThreshold) {
        throw new Error(`Need at least ${config.oldThreshold} old shares, got ${oldShares.length}`);
    }
    ...
    for (let j = 0; j < config.newParties.length; j++) {
        ...
        const newShare = reshareRound2(newPartyId, newPartyIndex, allRound1Messages, config, expectedPublicKey);
        newShares.push(newShare);
    }
    // Verification: all new shares must produce the same public key
    for (const share of newShares) {
        if (share.publicKey !== expectedPublicKey) {
            throw new Error("Resharing failed: new shares do not preserve the group public key");
        }
    }
    return newShares;
}
 ...
 */
export function proactiveRefresh(currentShares) {
    const config = {
