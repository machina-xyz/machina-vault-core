...
import { toHex, fromHex, randomBytes, modN, hashCommitment, } from "./commitment.js";
 ...
 */
export function keygenRound1(partyId, partyIndex, threshold, totalParties) {
    if (partyIndex < 1 || partyIndex > totalParties) {
        throw new Error(`Party index must be between 1 and ${totalParties}, got ${partyIndex}`);
    }
    if (threshold < 2 || threshold > totalParties) {
        throw new Error(`Threshold must be between 2 and ${totalParties}, got ${threshold}`);
    }
    ...
    const hc = hashCommitment(concatBytes(firstCommitBytes, chainCodeContribution));
    const state = {
        partyId,
        partyIndex,
        threshold,
 ...
 */
export function keygenRound2(state, round1Messages) {
    if (round1Messages.length !== state.totalParties) {
        throw new Error(`Expected ${state.totalParties} round 1 messages, got ${round1Messages.length}`);
    }
    // Parse and store all commitments
    const allCommitments = new Map();
    for (const msg of round1Messages) {
        const parsed = msg.commitments.map((hex) => Point.fromHex(fromHex(hex)));
        if (parsed.length !== state.threshold) {
            throw new Error(`Party ${msg.fromParty} sent ${parsed.length} commitments, expected ${state.threshold}`);
        }
    ...
    for (const msg of round1Messages) {
        if (msg.fromParty === state.partyId)
            continue;
        const shareValue = evaluatePolynomial(state.coefficients, BigInt(msg.fromIndex));
        round2Messages.push({
 ...
 */
export function keygenRound3(state, round2Messages) {
    // Filter messages addressed to us
    const myMessages = round2Messages.filter((msg) => msg.toParty === state.partyId);
    if (myMessages.length !== state.totalParties - 1) {
        throw new Error(`Expected ${state.totalParties - 1} round 2 messages for party ${state.partyId}, got ${myMessages.length}`);
    }
    ...
    for (const msg of myMessages) {
        ...
        if (!senderCommitments) {
            throw new Error(`No commitments found for party ${msg.fromParty}`);
        }
        ...
        const expectedPoint = verifyFeldmanShare(senderCommitments, BigInt(state.partyIndex));
        if (!sharePoint.equals(expectedPoint)) {
            throw new Error(`Share verification failed for share from party ${msg.fromParty}`);
        }
    ...
    let combinedShare = evaluatePolynomial(state.coefficients, BigInt(state.partyIndex));
    ...
    const { proofR, proofS } = generateSchnorrProof(combinedShare, state.partyId);
 ...
 */
export function verifyKeygenRound3(msg, allCommitments) {
    try {
        ...
        return verifySchnorrProof(claimedPublicShare, msg.proofR, msg.proofS, msg.fromParty);
    }
    catch {
        return false;
    }
 ...
 */
export function generateKeyShares(threshold, totalParties) {
    if (threshold < 2) {
    ...
    const round1Results = parties.map((p) => keygenRound1(p.id, p.index, threshold, totalParties));
    const round1Messages = round1Results.map((r) => r.round1Msg);
    // Round 2: All parties evaluate polynomials and send shares
    const round2Results = round1Results.map((r) => keygenRound2(r.state, round1Messages));
    ...
    const round3Results = round2Results.map((r) => keygenRound3(r.state, allRound2Messages));
 ...
 */
export function lagrangeCoefficient(i, indices) {
    let numerator = 1n;
    let denominator = 1n;
    const iBig = BigInt(i);
 ...
 */
function verifyFeldmanShare(commitments, x) {
...
function generateSchnorrProof(secret, context) {
...
function verifySchnorrProof(publicKey, proofR, proofS, context) {
    try {
