 ...
 */
export function checkSpendingLimit(key, tokenAddress, amount) {
    // Validate key status
    ...
    const spentToday = key.lastResetDay === today ? (key.spentToday[tokenAddress] ?? 0n) : 0n;
    const spentThisMonth = key.lastResetMonth === month
 ...
 */
export function recordSpend(key, tokenAddress, amount) {
 ...
 */
export function resetDailySpend(key) {
    const today = new Date().toISOString().slice(0, 10);
 ...
 * Returns a new VaultKey (immutable).
 */
export function resetMonthlySpend(key) {
    const month = new Date().toISOString().slice(0, 7);
 ...
 */
function findSpendingLimit(limits, tokenAddress) {
    // Exact match first
    const exact = limits.find((l) => l.tokenAddress.toLowerCase() === tokenAddress.toLowerCase());
    if (exact)
        return exact;
    // No match
    return null;
