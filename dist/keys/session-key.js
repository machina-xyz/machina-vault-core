 ...
 */
import { DEFAULT_SESSION_PERMISSIONS, constrainPermissions, validateKeyCreation, } from "./permissions.js";
 ...
 */
export function createSessionKey(params) {
    const { vaultId, name, parentKeyId, parentTier, parentPermissions, scope, ttlSeconds, } = params;
    ...
    // Clamp TTL
    const ttl = Math.min(Math.max(ttlSeconds ?? DEFAULT_SESSION_TTL_SECONDS, 1), MAX_SESSION_TTL_SECONDS);
    ...
    const effectivePermissions = constrainPermissions(parentPermissions, DEFAULT_SESSION_PERMISSIONS);
    const now = new Date();
