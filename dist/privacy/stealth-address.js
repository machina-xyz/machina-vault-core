 ...
 */
function pubKeyToAddress(uncompressedPubKey) {
 ...
 */
export function computeStealthMetaAddress(keyPair) {
    return {
 ...
 */
export function parseStealthMetaAddress(encoded) {
    const prefix = "st:eth:0x";
    if (!encoded.startsWith(prefix)) {
        throw new Error(`Invalid stealth meta-address format: must start with "${prefix}"`);
    }
    ...
    if (payload.length !== 132) {
        throw new Error(`Invalid stealth meta-address length: expected 132 hex chars, got ${payload.length}`);
    }
    ...
    try {
        ...
        throw new Error("Invalid stealth meta-address: public keys are not valid secp256k1 points");
    }
    return {
        spendingPubKey,
        viewingPubKey,
...
export function generateStealthAddress(metaAddress) {
    ...
    const viewingPoint = secp256k1.ProjectivePoint.fromHex(stripHexPrefix(metaAddress.viewingPubKey));
    const sharedSecretPoint = viewingPoint.multiply(bytesToBigInt(ephemeralKey));
    ...
    const spendingPoint = secp256k1.ProjectivePoint.fromHex(stripHexPrefix(metaAddress.spendingPubKey));
 ...
 */
export function checkStealthAddress(announcement, viewingKey) {
 ...
 */
export function computeStealthPrivateKey(spendingKey, viewingKey, ephemeralPubKey) {
    const n = secp256k1.CURVE.n;
    // 1. Compute shared secret: S_shared = v * R
    const ephemeralPoint = secp256k1.ProjectivePoint.fromHex(stripHexPrefix(ephemeralPubKey));
    const viewingKeyScalar = bytesToBigInt(hexToBytes(viewingKey));
    const sharedSecretPoint = ephemeralPoint.multiply(viewingKeyScalar);
    const sharedSecretBytes = sharedSecretPoint.toRawBytes(true);
