 ...
 */
export function generateAgentCard(identity, options) {
    const baseUrl = options?.baseUrl ?? `https://machina.money/agents/${identity.agentId}`;
    // Generate skills from agent capabilities
    const skills = options?.skills ?? generateDefaultSkills(identity.capabilities);
    return {
        name: identity.name,
 ...
 */
function generateDefaultSkills(capabilities) {
    const skillMap = {
        payment: {
            id: "machina_payment",
